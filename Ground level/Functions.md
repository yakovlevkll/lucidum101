...
В математике функции записывают с помощью букв <![if !msEquation]>  <![endif]> или <![if !msEquation]>  <![endif]>. Для того чтобы отличить функцию от обычной величины, после имени функции обычно пишут переменные, от которых она зависит. Например, функция <![if !msEquation]>  <![endif]>, которая зависит от <![if !msEquation]>  <![endif]> и <![if !msEquation]>  <![endif]> будет записана следующим образом
...
# Linear functions  (линейная функция)

<![if !msEquation]>  <![endif]>

<![if !msEquation]>  <![endif]> – коэффициент, отвечающий за скорость подъема прямой. Численно равен тангенсу угла наклона прямой (если тангенс не известен читатель, то можно пока просто запомнить, что коэффициент связан с углом наклона). Буква <![if !msEquation]>  <![endif]> выбрана не просто так – позже мы узнаем, что данное число также является градиентом.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDU2NjU3MjI1LC0xMzgzMjI3MTM1XX0=
-->